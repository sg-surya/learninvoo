        ...
        <div className="h-screen flex flex-col bg-background font-sans text-foreground">
            ...
            <header className="sticky top-0 z-50 h-16 bg-card-bg/95 backdrop-blur-md border-b border-border px-6 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-sm font-bold text-foreground">{currentChapter.title}</h1>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-muted">
                    {/* Sticky Toolbar */}
                    <div className="sticky top-0 z-40 bg-card-bg border-b border-border shadow-sm">
                        {/* Main Controls */}
                        <div className="h-14 flex items-center justify-between px-6">
                            <div className="flex items-center gap-4">
                                <span className="text-sm text-muted-foreground font-medium">Page</span>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                                        disabled={currentPage <= 1}
                                        className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                        title="Previous Page (←)"
                                    >
                                        <ChevronLeft size={16} />
                                    </button>
                                    <input
                                        ...
                                        min={1}
                                        max={numPages || 999}
                                        className="w-16 bg-muted text-foreground text-sm text-center px-2 py-1.5 rounded border border-border focus:border-primary-custom focus:outline-none focus:ring-2 focus:ring-primary-custom/20 font-medium"
                                    />
                                    <button
                                ...
                                <span className="text-sm text-muted-foreground font-medium">of {numPages || '...'}</span>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-foreground font-medium w-12 text-center">{zoom}%</span>
                                <button
                    ...
                    {/* PDF Canvas */}
                    <div className="flex-1 overflow-auto bg-muted flex items-start justify-center p-8 custom-scrollbar">
                        {loadingPdf ? (
                            <div className="flex flex-col items-center justify-center p-12">
                                <Loader2 size={48} className="text-primary-custom animate-spin mb-4" />
                                <p className="text-muted-foreground font-medium">Loading PDF from storage...</p>
                            </div>
                        ) : pdfError ? (
                            <div className="flex flex-col items-center justify-center p-12 bg-card-bg rounded-lg shadow-lg border border-border">
                                <BookOpen size={64} className="text-muted-foreground/30 mb-4" />
                                <h3 className="text-xl font-bold text-foreground mb-2">PDF Not Available</h3>
                                <p className="text-muted-foreground text-sm mb-4">{pdfError}</p>
                                <Link
                                    href={`/library/${book.id}/manage`}
                                    className="px-4 py-2 bg-primary-custom text-white rounded-lg font-medium hover:opacity-90 transition-colors"
                                >
                                    Go to Manage Book
                                </Link>
                            </div>
                    ...
                    <div className="h-12 bg-card-bg border-t border-border px-6 flex items-center justify-center gap-2">
                        <button
                            onClick={() => setCurrentPage(1)}
                            disabled={currentPage === 1}
                            className="px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                        >
                            First
                        </button>
                        <button
                ...
                {/* Chapter Sidebar */}
                <aside className="w-80 bg-card-bg border-l-2 border-border overflow-y-auto custom-scrollbar shadow-sm">
                    <div className="p-6">
                            ...
                            {book.chapters.map((chapter, idx) => (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${idx === chapterIndex ? 'bg-white/20 text-white' : 'bg-background text-muted-foreground'
                                            }`}>
                                            {idx + 1}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-bold text-sm mb-1 truncate">{chapter.title}</h4>
                                            <p className={`text-xs ${idx === chapterIndex ? 'text-white/80' : 'text-muted-foreground'}`}>
                                                {chapter.pages} pages
                                            </p>
                                        </div>
                                    </div>
